...
```
Browser ──HTTPS──▶ Cloudflare ──HTTPS + mTLS──▶ Caddy ──HTTP──▶ Next.js (Bun, :7004)
                                                               │
...
- **源站**：自托管 VPS，Caddy 终止 TLS（使用 Cloudflare Origin Certificate），并启用 **Authenticated Origin Pulls** (mTLS)，拒绝直连 IP 的流量
...
2. 一个工作目录（示例 `/opt/noheir/`），内含：
   - `docker-compose.yml`：两个 service —— `caddy` 和 `app`，共享一个内部 network；`app` 拉取 `ghcr.io/<owner>/noheir:latest`
   - `Caddyfile`：站点指向 `reverse_proxy app:7004`，并配置 `tls <origin-cert> <origin-key>` + `client_auth` 信任 Cloudflare 的 `authenticated_origin_pull_ca.pem`
   - `.env`（chmod 600）：上表中的所有运行时变量
   - `certs/`：Cloudflare Origin Certificate、私钥、CF Authenticated Origin Pull CA 证书
3. SSH 公钥：单独生成一对部署密钥，公钥放进 `~/.ssh/authorized_keys`，私钥配进 GitHub Actions secret
4. 防火墙：仅放行 22（部署用，可按需收紧到 GitHub Actions IP 段或临时开启）和 443
...
2. `appleboy/ssh-action` 进入源站执行：
   ...
   docker compose up -d --no-deps app   # 不动 caddy，避免无谓重启
