...
|---|--------|-------------|-------|
| 0.1a | `chore: archive Gen 1 codebase to _archive/` | Move (not delete) all Gen 1 source into `_archive/` for reference during rewrite. Old code stays accessible for copy-paste and comparison. `_archive/` is gitignored after Phase 5 cutover and deleted once rewrite is verified. | See "Phase 0.1a: Archive Scope" below |
| 0.1b | `feat: scaffold Next.js 16 project` | `create-next-app` with App Router, React 19, Tailwind v4, TypeScript strict. Copy design tokens from `_archive/src/index.css` to `app/globals.css`. Configure `next.config.ts` (standalone output). | `package.json`, `next.config.ts`, `tsconfig.json`, `app/globals.css`, `app/layout.tsx` |
| 0.2 | `feat: add Gen 2 layout system` | Port `_archive/src/components/layout/DashboardLayout.tsx` (539 lines) → `app-shell.tsx` + `sidebar.tsx` + `sidebar-context.tsx`. Extract nav config to `lib/navigation.ts`. | `components/layout/{app-shell,sidebar,sidebar-context,index}.tsx`, `lib/navigation.ts` |
