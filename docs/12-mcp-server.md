...
`mcp/` 目录是一个独立的 MCP（Model Context Protocol）服务端，通过 stdio 传输协议向 AI Agent（如 Claude Desktop）暴露 **只读** 的财务数据查询能力。

Agent 可以通过 4 个工具查询收支交易、内部转账、财务元数据和月度报表，无需直接操作数据库。
...
├── tests/
│   ├── tools.e2e.test.ts        # 59 个工具处理函数测试
│   └── mcp.e2e.test.ts          # 7 个协议层端到端测试
...
- **只读**：4 个工具全部为查询操作，不暴露任何写入能力。
- **服务端聚合**：所有数据通过 Postgres RPC 函数获取，避免 PostgREST `max_rows`（默认 1000）限制。
...
|------|------|--------|---------|-----------|
| 工具处理 E2E | `tools.e2e.test.ts` | 59 | 直接调用处理函数 + 真实 Supabase | 是（本地） |
| 协议 E2E | `mcp.e2e.test.ts` | 7 | 子进程 stdio 完整 JSON-RPC 往返 | 是（本地） |

### 运行命令

```bash
# 从项目根目录运行（推荐）
bun run test:mcp    # 全部 66 个测试
