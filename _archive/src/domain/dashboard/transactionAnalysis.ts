...
export const buildFilteredTransactions = (transactions: Transaction[], type: TransactionType) => {
  return transactions.filter(t => t.type === type);
};

export const buildTotalAmount = (transactions: Transaction[]) => {
  return transactions.reduce((sum, t) => sum + t.amount, 0);
};

export const buildTopTransactions = (transactions: Transaction[], limit = 50) => {
...
export const buildMonthlyFiltered = (monthlyData: MonthlyData[], type: TransactionType) => {
  return monthlyData.filter(d => (type === 'income' ? d.income : d.expense) > 0);
};
