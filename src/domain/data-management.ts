 ...
 */
export function buildDataHealthMetrics(transactions: DomainTransaction[]): DataHealthMetrics {
...
export function buildDataSummary(
  ...
  const yearlyStats = years.map((year) => buildYearlyStats(transactions, transfers, year));
...
export function getHealthScore(metrics: DataHealthMetrics): number {
  ...
  const issueCount =
    metrics.missingDates + metrics.futureDates + metrics.zeroAmounts + metrics.negativeAmounts;

  const integrityScore = ((metrics.totalRecords - issueCount) / metrics.totalRecords) * 100;
