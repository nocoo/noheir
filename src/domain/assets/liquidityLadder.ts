...
export const buildMonthlyMaturities = (units: UnitDisplayInfo[], monthsAhead = 24) => {
  if (!units || units.length === 0) {
    return { monthlyMaturities: [] as MonthlyMaturity[], strategies: [] as string[], months: [] as string[] };
  }

  const establishedUnits = units.filter(unit => unit.status === '已成立' && unit.end_date && unit.product);
...
export const buildSeries = (monthlyData: { monthlyMaturities: MonthlyMaturity[]; strategies: string[]; months: string[] }) => {
  return monthlyData.strategies.map(strategy => ({
    name: strategy,
    type: 'bar',
    stack: 'total',
    data: monthlyData.months.map(month => {
      const item = monthlyData.monthlyMaturities.find(m => m.month === month && m.strategy === strategy);
      return item?.amount || 0;
    }),
  }));
};

export const buildSummaryStats = (monthlyData: { monthlyMaturities: MonthlyMaturity[]; months: string[] }) => {
