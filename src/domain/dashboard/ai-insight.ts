...
export const sortRecurringPaymentsByNextDate = (
  payments: RecurringPayment[],
) => {
  return [...payments].sort((a, b) => {
    const daysA = Math.abs(
      new Date(a.nextPaymentDate).getTime() - Date.now(),
    );
    const daysB = Math.abs(
      new Date(b.nextPaymentDate).getTime() - Date.now(),
    );
    return daysA - daysB;
  });
};
