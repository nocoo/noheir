...
import type { Transfer } from '@/types/data';
import type { AccountType, AccountTypeConfig, BalanceAnchor } from '@/contexts/SettingsContext';
...
export interface BalanceEntry {
  ...
  tertiaryCategory?: string;
  description?: string;
  originalData: Transaction | Transfer;
}

type AnchorWithMeta = BalanceAnchor & { isStarting?: boolean };

export const buildBalanceEntries = (transactions: Transaction[], transfers: Transfer[]): BalanceEntry[] => {
...
export const buildUniqueAccounts = (entries: BalanceEntry[]) => {
...
export const buildAccountDetailData = (
  ...
  const accountAnchors = balanceAnchors
    ?.filter(a => a.accountName === selectedAccount)
    .sort((a, b) => a.date.localeCompare(b.date)) || [];

  const yearStartDate = `${selectedYear}-01-01`;
  const yearEndDate = `${selectedYear}-12-31`;
  const displayAnchors = accountAnchors.filter(a => a.date >= yearStartDate && a.date <= yearEndDate);
  ...
  for (let i = 0; i < yearAnchors.length; i++) {
    ...
    if (!currentAnchor.isStarting && currentAnchor.date >= yearStartDate && currentAnchor.date <= yearEndDate) {
      displayEntries.push({
