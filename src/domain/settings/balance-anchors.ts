import type { BalanceAnchor } from "../types";
...
export const groupAnchorsByAccount = (anchors: BalanceAnchor[]) => {
  ...
  anchors.forEach((anchor) => {
    ...
    const group = grouped[anchor.accountName];
    if (group) group.push(anchor);
  });

  Object.keys(grouped).forEach((account) => {
    const group = grouped[account];
    if (group) group.sort((a, b) => b.date.localeCompare(a.date));
  });
...
export const calculateBalanceAtDate = (params: {
  ...
  const difference =
    inputBalance !== null && inputBalance !== undefined
