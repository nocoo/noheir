                    ...
                    return (
                      <div className="rounded-lg border border-border/50 bg-background px-3 py-2 text-xs shadow-xl">
                        ...
                        <p className="text-muted-foreground">
                          {formatCurrencyFull(d.value)} ({d.percentage.toFixed(1)}%)
                        </p>
                      </div>
                    );
                  }}
          ...
          {data.map((item, i) => (
            <div key={item.name} className="flex items-center gap-1 text-xs">
              <div
                ...
                style={{
                  backgroundColor: PIE_COLORS[i % PIE_COLORS.length] ?? PIE_COLORS[0] ?? "#64748b",
                }}
              />
...
export function CapitalDashboardClient({ units, deploymentRate }: CapitalDashboardClientProps) {
  // Filter state
  ...
  const activeFilterCount = [filterStatus, filterStrategy, filterTactics, filterCurrency].filter(
    (f) => f !== "all",
  ).length;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <StatCard
          ...
          variant="expense"
        />
        <StatCard title="闲置单位" value={`${idleUnitsCount}个`} icon={Clock} variant="warning" />
      </div>
          ...
          {Object.entries(totalsByCurrency).map(([currency, amount]) => (
            <Card key={currency} className="border-l-4 border-l-primary">
              <CardContent className="flex items-center justify-between p-3">
                ...
                <span className="text-primary font-bold">{formatCurrencyFull(amount)}</span>
              </CardContent>
            </Card>
          ))}
        </div>
