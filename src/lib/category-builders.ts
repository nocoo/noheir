...
export function buildCategoryData(
  ...
  const processedPrimary: PrimaryCategoryGroup[] = Array.from(
    primaryMap.entries(),
  )
    .map(([primary, data]) => {
      const secondaryCategories: SecondaryCategory[] = Array.from(
        data.secondaryMap.entries(),
      )
        .map(([secName, secData]) => {
          const tertiaryList: TertiaryCategory[] = Array.from(
            secData.tertiaryMap.entries(),
          )
            .map(([name, td]) => ({
              name,
              total: td.total,
  ...
  const majorPrimaries = processedPrimary.filter(
    (c) => c.percentage >= THRESHOLD,
  )
  const smallPrimaries = processedPrimary.filter(
    (c) => c.percentage < THRESHOLD,
  )
