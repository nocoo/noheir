...
describe("parseChineseCSV", () => {
  ...
  test("parses a valid expense row", () => {
    const csv = [header, "2025-03-15,支出,吃饭,0.00,35.50,人民币,支付宝,,午餐"].join("\n");
  ...
  test("parses a valid income row", () => {
    const csv = [header, "2025-01-10,收入,工资,25000.00,0.00,人民币,招商银行,,一月薪资"].join("\n");
    const result = parseChineseCSV(csv);
    expect(result.transactions).toHaveLength(1);
  ...
  test("converts decimal amounts to cents correctly", () => {
    const csv = [header, "2025-06-01,支出,超市,0.00,123.45,人民币,微信,,"].join("\n");
    const result = parseChineseCSV(csv);
    expect(result.transactions[0]?.amountCents).toBe(12345);
  });

  test("handles 余额调整 → 对账收入/对账支出", () => {
    const csvIncome = [header, "2025-01-01,收入,余额调整,100.00,0.00,人民币,支付宝,,"].join("\n");
    const r1 = parseChineseCSV(csvIncome);
    expect(r1.transactions[0]?.tertiaryCategory).toBe("对账收入");

    const csvExpense = [header, "2025-01-01,支出,余额调整,0.00,50.00,人民币,支付宝,,"].join("\n");
  ...
  test("skips zero-amount rows with warning", () => {
    const csv = [header, "2025-01-01,支出,吃饭,0.00,0.00,人民币,支付宝,,"].join("\n");
    const result = parseChineseCSV(csv);
  ...
  test("returns error for invalid date", () => {
    const csv = [header, "not-a-date,支出,吃饭,0.00,10.00,人民币,支付宝,,"].join("\n");
    const result = parseChineseCSV(csv);
    expect(result.errors).toHaveLength(1);
  ...
  test("parses tags as JSON string", () => {
    const csv = [header, "2025-01-01,支出,吃饭,0.00,20.00,人民币,支付宝,日常,聚餐,,"].join("\n");
    const result = parseChineseCSV(csv);
  ...
  test("handles quoted fields with commas", () => {
    const csv = [header, '2025-01-01,支出,吃饭,0.00,20.00,人民币,支付宝,"标签A,标签B",备注'].join(
      "\n",
    );
    const result = parseChineseCSV(csv);
    expect(result.transactions).toHaveLength(1);
  ...
  test("handles Windows CRLF line endings", () => {
    const csv = [header, "2025-01-01,支出,吃饭,0.00,20.00,人民币,支付宝,,"].join("\r\n");
    const result = parseChineseCSV(csv);
    expect(result.transactions).toHaveLength(1);
  });

  test("marks unmapped categories with has_secondary_mapping = 0", () => {
    const csv = [header, "2025-01-01,支出,不存在的类型,0.00,10.00,人民币,支付宝,,"].join("\n");
    const result = parseChineseCSV(csv);
    expect(result.transactions).toHaveLength(1);
