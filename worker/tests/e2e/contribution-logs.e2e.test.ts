...
describe("E2E: Contribution Logs", () => {
  ...
  test("POST /api/contribution-logs/seed is idempotent", async () => {
    const { product } = await api<{ product: Record<string, unknown> }>({
      method: "POST",
      path: "/api/products",
      userId,
    ...
    // Create unit with product and startDate
    const { unit } = await api<{ unit: Record<string, unknown> }>({
      method: "POST",
      path: "/api/units",
      userId,
      body: makeUnit({ productId: product.id, startDate: "2026-01-01" }),
    });
    ...
    const first = await api<{
      ...
      method: "POST",
      path: "/api/contribution-logs/seed",
      userId,
    });
    ...
    // Run seed second time (should skip)
  ...
  test("POST /api/contribution-logs/seed skips units without product", async () => {
